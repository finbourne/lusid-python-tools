...
def parse(with_inputs=True,args=None):
    parser = argparse.ArgumentParser(description='DataFrame Query Tool',fromfile_prefix_chars='@')
    if with_inputs:
    ...
    parser.add_argument('-w','--where', nargs='*', metavar='column=value',
                        help='filtering')
    parser.add_argument('-o','--order', nargs='*', metavar='column',
    ...
    parser.add_argument('--tab', action='store_true', help='tab separator for text files')
    parser.add_argument('--latin', action='store_true', help='read files with latin-1 encoding')
    parser.add_argument('--markdown', action='store_true', help='give output in markdown format')
    parser.add_argument('--count', action='store_true', help='count records in groups')
    parser.add_argument('--single', nargs='+', help='Single column uniqueness constraint.')
...
def dfq(args,given_df=None):
    ...
    nrows = args.first if args.first > 0 and args.last == 0 and args.groupby is None and args.where is None and args.order is None and len(args.input) == 1 else None
    ...
    if args.where:
       for c in args.where:
           ...
           if len(s) == 1:
              if '*' in kv[1]:
                 crit = df[col].astype(str).fillna('').str.match("({})".format(kv[1].replace('*','.*')))
              else:
                 dflt=''
                 v = kv[1]
                 if df[col].dtype==int:
