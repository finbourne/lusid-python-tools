...
def group_request_into_one(model_type, request_list: list, attribute_for_grouping: list, batch_index=0):
    ...
    for attrib in attribute_for_grouping:
        ...
        if getattr(models, model_type).openapi_types[attrib] == 'list[ResourceId]':
            ...
            batch_attrib = [attrib for nested_list in [attrib.values for attrib in request_list if attrib.values is not None] for attrib in nested_list]

            # Assign collated values onto the base request

            setattr(base_request, "values", batch_attrib)

        elif getattr(models, model_type).openapi_types[attrib] == 'dict(str, ModelProperty)':

            # Collect the attributes from each request onto a dictionary

            batch_attrib = dict([(attrib, nested_list[attrib]) for nested_list in [
                attrib.properties for attrib in request_list if attrib.properties is not None] for attrib in nested_list])
