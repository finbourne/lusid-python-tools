...
class CocoonTestsExtractGroupHoldings(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        portfolio_holdings = pd.read_csv(Path(__file__).parent.joinpath("./data/holdings-example.csv"))
        portfolio_groups = pd.read_csv(Path(__file__).parent.joinpath("./data/portfolio-groups.csv"))
        ...
        response = cocoon.load_from_data_frame(
            ...
            mapping_optional={"values.scope": f"${cls.scope}", "values.code": "FundCode"},
        ...
        response = cls.api_factory.build(lusid.api.PortfolioGroupsApi).create_portfolio_group(
            scope=cls.scope,
    ...
    def check_holdings_correct(self, group_holdings, expected_values, lusid_results_keyed):
        ...
        if lusid_results_keyed is not None:
            self.assertEqual(set(group_holdings.keys()), set(lusid_results_keyed.keys()))
        ...
        for portfolio, holdings in group_holdings.items():
            ...
            holdings_by_instrument_name = {
                holding.properties["Instrument/default/Name"].value.label_value: holding for holding in holdings
            }
            ...
            for instrument_name, holding in holdings_by_instrument_name.items():
                self.assertEqual(float(holding.cost.amount), float(expected_values[portfolio][instrument_name]['cost']))
                self.assertEqual(float(holding.units), float(expected_values[portfolio][instrument_name]['units']))
                self.assertEqual(holding.cost.currency, expected_values[portfolio][instrument_name]['cost.currency'])
                ...
                self.assertEqual(float(holding.cost.amount), float(lusid_results_keyed[portfolio][instrument_name].cost.amount))
                self.assertEqual(float(holding.units), float(lusid_results_keyed[portfolio][instrument_name].units))
                self.assertEqual(holding.cost.currency, lusid_results_keyed[portfolio][instrument_name].cost.currency)
                self.assertEqual(holding.cost_portfolio_ccy.currency, lusid_results_keyed[portfolio][instrument_name].cost_portfolio_ccy.currency)
                self.assertEqual(float(holding.cost_portfolio_ccy.amount), float(lusid_results_keyed[portfolio][instrument_name].cost_portfolio_ccy.amount))
    ...
    def test_get_holdings_for_group_grouped_by_portfolio(self, _, group_code, group_by_portfolio, expected_results) -> None:
        ...
        lusid_results = self.api_factory.build(lusid.api.PortfolioGroupsApi).get_holdings_for_portfolio_group(
            scope=self.scope,
            code=group_code,
            property_keys=["Instrument/default/Name"]
        )
        ...
        for holding in lusid_results.values:
            portfolio = holding.properties["Holding/default/SourcePortfolioId"].value.label_value.replace("/", " : ")
            instrument_name = holding.properties["Instrument/default/Name"].value.label_value
            lusid_results_keyed.setdefault(portfolio, {}).setdefault(instrument_name, holding)

        # Check that the result is as expected
        self.check_holdings_correct(group_holdings, expected_results, lusid_results_keyed)
